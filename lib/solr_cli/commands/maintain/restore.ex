defmodule SolrCli.Commands.Maintain.Restore do
  ...
  def run(%{solr: solr, repository: repository, location: location}, %{suffix: suffix, reference: reference}, %{config: %{"url" => urls}}) do
  ...
  def run(%{solr: solr, repository: repository, location: location}, %{suffix: suffix, collections: c9s}, %{config: %{"url" => urls}}) do
    solr_client = HttpClient.new(base_url: urls[solr])
  ...
  defp do_run(solr_client, repository, location, collections, suffix) do
    ...
    collections
    |> Stream.map(& "/admin/collections?action=RESTORE&name=#{&1}#{suffix}&repository=#{repository}&location=#{location}&collection=#{&1}&async=#{timestamp}_#{&1}")
