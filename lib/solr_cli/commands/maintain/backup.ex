defmodule SolrCli.Commands.Maintain.Backup do
  ...
  def run(%{solr: solr, repository: repository, location: location }, %{suffix: suffix}, %{config: %{"url" => urls}}) do
    solr_client = HttpClient.new(base_url: urls[solr])
  ...
  defp do_run(solr_client, repository, location, collections, suffix) do
    ...
    collections
    |> Stream.map(& "/admin/collections?action=BACKUP&name=#{&1}#{suffix}&repository=#{repository}&location=#{location}&collection=#{&1}&async=#{timestamp}_#{&1}")
