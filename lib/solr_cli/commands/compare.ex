defmodule SolrCli.Commands.Compare do
  use DoIt.Command,
  ...
  def run(
    ...
    IO.puts("#{DateTime.to_iso8601(DateTime.utc_now())} - Fetching configuration from Solr #{urls[solr_source]}...")
    solr_source_client
    ...
    |> Enum.each(&Controller.request(solr_target_client, &1))
    #|> Enum.each(&IO.puts/1)
